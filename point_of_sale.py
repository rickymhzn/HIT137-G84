...
# Question 2 – Python GUI Interface for Point of Sale
...
class POS:
    def __init__(self,window):
        ...
        MenuFrame = Frame(MainFrame, bg="white", bd=3 , width=1000,height=760, padx=5, pady=5, relief=RIDGE)
        MenuFrame.pack(side=LEFT)
        BottomFrame = Frame(MenuFrame, bg="white", bd=3 , width=1000,height=220, padx=5, pady=5, relief=RIDGE)
        ...
        def payment_type_selected(selection):
            ...
            if(selection == 'Card'):
                cash_input.set((total_cost)+(total_cost*tax)/100)
                change_input.set("0.00")
            else:
                cash_input.set("")
                change_input.set("0.00")
        ...
        self.payment_type.config(width=20,bd=1)
